...
- A real in-app payment provider is the only external integration still pending for the escrow/checkout flow — the Bible names Bind (`BindFinancialProvider`) as the intended candidate, not MercadoPago (which is a separate manual deposit/withdrawal flow, not an app integration); the code side is ready (see Financial integration status above).

## Recommended next steps

1. **Connect Bind (or an equivalent BaaS/CVU provider)** as the real in-app payment provider — the highest-leverage next step; unblocks the Bible's funding-method commission matrix (10/15/20%) and makes the whole financial stack real instead of simulated. Not MercadoPago (see Financial integration status).
