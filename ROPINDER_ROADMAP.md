...
|---|---|---|
| Conectar un proveedor de pago in-app real (Fase 15) | Decisión/cuenta del usuario | **Corrección**: MercadoPago NO es este proveedor — es solo el mecanismo manual por el que la plata entra/sale de la cuenta del negocio (el admin confirma la transferencia y acredita créditos a mano desde `/admin/herramientas`), no algo con lo que el checkout/escrow de la app hable directamente. El proveedor real para `getFinancialProvider()` (cobrar/pagar dentro del flujo de compra) sigue sin definirse — Stripe (única opción del Vercel Marketplace) no soporta payouts a Argentina, así que probablemente sea otro gateway, a decidir. La arquitectura ya está lista (`FinancialProviderAdapter`, Ciclo 5) — conectarlo es implementar una clase nueva y cambiar `getFinancialProvider()`, no tocar los call sites. Sigue bloqueando la matriz de comisión por método de pago (10/15/20), que necesita plata de verdad entrando para tener sentido. |
