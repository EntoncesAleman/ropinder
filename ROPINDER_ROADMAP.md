...
## Ciclo 7 — Loop 06 (Monetización): Tiendas, VIP, insignia anual, retiro Premium, stats, alertas, multi-foto

Disparado por una revisión conjunta de precios contra la competencia real (Vinted, Depop, Poshmark, Mercari, Vestiaire, Tinder) y contra indicadores económicos argentinos (dólar blue, salario mínimo agosto 2026) — ver la conversación para el detalle de la investigación. De ahí salieron 8 decisiones de producto, todas implementadas:

1. **Precios revertidos a default** — un ×10 accidental en los 8 productos de `/admin/precios` (probablemente al probar el panel) se corrigió borrando los overrides `pricing.*` de `Config` en producción (verificado antes/después: 8 filas borradas, las 3 de comisión/retiro intactas).
2. **Cuentas Tienda**: `User.accountType` (`PERSONAL` | `STORE`), sección `/admin/tiendas` separada de `/admin/usuarios` (cada una filtra la otra), botón para convertir en cualquiera de las dos listas. `lib/pricing.ts` gana un segundo nivel de `Config` (`pricing.<pack>.STORE`) que cascadea sobre el precio personal si no se fijó uno propio — `/admin/precios` tiene un selector Personal/Tienda.
3. **Insignia verificada anual**: nuevo campo `verifiedUntil`, mismo patrón de expiración perezosa que `premiumUntil` (`lib/auth.ts`). Comprarla/renovarla apila un año sobre el tiempo restante, igual que Premium. Migración con backfill: usuarios ya verificados reciben `verifiedAt + 1 año` en vez de quedar sin vencimiento.
4. **Retiro 2 días para Premium** (antes 3 para todos): `lib/withdrawal.ts` gana `withdrawalHoldHours(isPremium)` — Premium espera 24h en vez de 48h antes de que arranque la ventana de 24h de fee, así su plata queda libre de comisión un día antes.
5. **Listados VIP**: la corrección más grande de diseño de esta tanda. Antes, comprar una prenda requería que *ambos* lados se hubieran dado like — un requisito raro para un marketplace. Ahora un vendedor paga créditos para publicar como VIP (`ClothingItem.isVip`) y un comprador paga créditos para desbloquearla, lo que crea el `Match` directo sin pasar por el doble-like (`lib/match.ts`'s `findOrCreateMatch()`, compartido con el swipe normal para no duplicar lógica). Costos configurables desde el panel de Comisiones (`vip.publishCost`/`vip.unlockCost`).
6. **Estadísticas de vendedor**: `viewCount` ya existía pero no se mostraba en Ropero — se sumó `likesCount` (swipes con like) y `favoritesCount` vía `_count` filtrado de Prisma, sin tabla nueva.
7. **Alertas de bajada de precio**: nuevo `PATCH /api/clothes/[id]` (edición de precio únicamente, no hay edición de item completa todavía) que notifica a todo el que le dio like al ítem cuando el precio baja — reutiliza `Swipe` como lista de interesados, no hizo falta modelo nuevo.
8. **Búsquedas guardadas**: modelo `SavedSearch` nuevo (`lib/savedSearch.ts` tiene el matching puro y testeado), página `/busquedas-guardadas`, dispara notificación cuando se publica una prenda nueva que coincide (enganchado en `POST /api/clothes`).
9. **Multi-foto**: `ClothingItem.images` (JSON array) además de `imageUrl` (que sigue siendo la portada = `images[0]`, así ningún render existente — feed, Ropero, admin — necesitó tocarse). Formulario de publicación sube hasta 6 fotos; la página de detalle tiene carrusel con puntos cuando hay más de una.

No incluido en esta tanda (fuera de alcance explícito o necesita su propia pasada): edición completa de una publicación ya creada (solo precio es editable hoy), badge de VIP en el deck de swipe (bump tampoco se muestra ahí, se mantuvo consistencia). El costo de créditos VIP mostrado en los botones de publicar/desbloquear estaba hardcodeado al lanzar esta sección — se corrigió en el commit siguiente (`/api/pricing` ahora también devuelve `vipPublishCost`/`vipUnlockCost` en vivo).

Verificación: `tsc --noEmit` limpio, `eslint` limpio (proyecto completo), 54/54 tests unitarios, 19/19 tests de integración (`npm run test:integration`, incluye `findOrCreateMatch` y el cascadeo de precios por tienda contra una base real). Migración Turso nueva (`scripts/sync-monetization-schema-turso.ts`) con backfill de `verifiedUntil` e `images` — pendiente de autorización explícita antes de correr contra producción.

## Backlog priorizado (ciclos futuros)

| Ítem | Bloqueado por | Motivo |
|---|---|---|
| Conectar un proveedor de pago in-app real (Fase 15) | Decisión/cuenta del usuario | **Corrección**: MercadoPago NO es este proveedor — es solo el mecanismo manual por el que la plata entra/sale de la cuenta del negocio (el admin confirma la transferencia y acredita créditos a mano desde `/admin/herramientas`), no algo con lo que el checkout/escrow de la app hable directamente. **La propia Biblia (`ROPINDER_BIBLE/LOOPS/01-FINTECH-WALLET.md` §8 y `00-ROPINDER-BIBLE.md` §9) ya nombra el candidato**: junto a `MockFinancialProvider`, prepara explícitamente `BindFinancialProvider` — Bind (Bind Banco Industrial), un proveedor BaaS/CVU argentino, exactamente el tipo de proveedor que evita el registro como PSPCP ante el BCRA (ver la sección "Decisión legal" arriba). Sigue siendo una decisión/cuenta del usuario (crear la cuenta comercial en Bind, conseguir credenciales), pero ya no está "por decidir" — el próximo paso concreto es esa cuenta, no elegir entre proveedores. La arquitectura ya está lista (`FinancialProviderAdapter`, Ciclo 5) — conectarlo es implementar una clase `BindFinancialProvider` nueva y cambiar `getFinancialProvider()`, no tocar los call sites. Sigue bloqueando la matriz de comisión por método de pago (10/15/20), que necesita plata de verdad entrando para tener sentido. |
...
| Video por publicación (solo fotos por ahora, Loop 02) | — | Multi-foto ya está (Ciclo 7) — video es un formato distinto (procesamiento, límites de tamaño, player) que no se resolvió de paso. |
| Edición completa de una publicación ya creada | — | Hoy solo el precio es editable (Ciclo 7, para habilitar alertas de bajada). Editar título/descripción/fotos de algo ya publicado necesita su propio formulario. |
...
## Decisión legal — por qué el saldo/créditos de Ropinder NO son una cuenta de pago (BCRA)

Riesgo identificado: si la base de datos guarda un número en pesos que representa dinero real depositado en una cuenta bancaria a nombre de un usuario específico, el BCRA clasifica a la plataforma como **PSPCP** (Proveedor de Servicios de Pago que ofrece Cuentas de Pago — el mismo régimen de Mercado Pago, Lemon, Personal Pay), con inscripción en el Registro del BCRA, regímenes informativos, auditorías SEFYC y la obligación de mantener el 100% de los fondos en cuentas a la vista sin poder invertirlos.

**Solución (estándar en apps de economía circular tipo Vinted/Wallapop) — ya es como está construido Ropinder, no requirió rediseño:**

- `User.balance` ("Ropinder Cash") y `User.credits` nunca representan un depósito bancario segregado por usuario — son créditos de consumo interno, filas de una base de datos, respaldados por una única cuenta de recaudación/custodia de la plataforma (no una cuenta por usuario). Esto es justo lo que ya hace `lib/financialProvider.ts`: el dinero real (cuando haya un proveedor conectado) entra a una cuenta de la plataforma vía `charge()`, y el balance del usuario es solo un asiento contable interno hasta que pide `payout()`.
- El dinero real recién se mueve en el momento del **cash-out** (`POST /api/transactions/withdraw`) — ahí es donde correspondería la liquidación real hacia el CBU/CVU del usuario, no antes. Reusar plata dentro de la app (comprar con `balance`, gastar `credits` en swipes/VIP/bump) nunca mueve dinero bancario real, solo actualiza filas.
- **Gap cerrado este ciclo**: los Términos (`/terms`, sección 5) no tenían la aclaración legal explícita. Se agregó: *"Los saldos y créditos... representan créditos de consumo interno... no constituyen depósitos en cuenta bancaria... no son una cuenta de pago... el dinero real que los respalda se mantiene en una cuenta de recaudación de la plataforma hasta el momento en que el usuario solicita transferirlo a su propio CBU/CVU."*
- **Pendiente, no es un cambio de código**: el esquema real de dos cuentas bancarias (Cuenta Custodia/Recaudación a nombre de la empresa, donde entra la plata de compradores/cargas; Cuenta Operativa/Comisiones, donde se consolida solo lo retenido en cada retiro) es una decisión bancaria/contable que el usuario tiene que resolver con su banco/PSP cuando conecte el proveedor de pago real (ver "Conectar un proveedor de pago in-app real" en el backlog) — el código ya está preparado para separarlo (`getFinancialProvider()` es el único punto de integración), pero las cuentas en sí no son algo que se "programe".
- **Nota**: hoy la comisión de venta se descuenta en el momento en que se libera el escrow (no en el momento del retiro, a diferencia del ejemplo que describe el retiro como el único punto de cobro) — esto no cambia el argumento legal (sigue sin haber depósito bancario individual hasta el cash-out), pero si se quiere trazabilidad contable más prolija para facturar automáticamente por comisión, valdría la pena en un ciclo futuro sumar un balance/ledger explícito de "ingresos de la plataforma" separado de los balances de usuario (hoy la comisión de cada venta queda registrada en el `meta` de la `Transaction`, pero no se acumula en ningún lado agregable de un vistazo).
