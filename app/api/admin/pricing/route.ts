...
import { PACKS, PackId, AccountType, getEffectivePacks, setPackPrice } from "@/lib/pricing";

function readAccountType(req: NextRequest): AccountType {
  return req.nextUrl.searchParams.get("accountType") === "STORE" ? "STORE" : "PERSONAL";
}
...
export async function GET(req: NextRequest) {
  ...
  const accountType = readAccountType(req);
  const effective = await getEffectivePacks(accountType);
...
export async function PATCH(req: NextRequest) {
  ...
  await Promise.all(edits.map((e) => setPackPrice(e.id, e.price, admin.id, accountType)));

  logAdminAction(admin.id, "PACK_PRICE_UPDATED", "Config", null, { accountType, ...Object.fromEntries(edits.map((e) => [e.id, e.price])) }).catch(() => {});
