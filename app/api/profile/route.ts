...
export async function PATCH(req: NextRequest) {
  ...
  const { name, avatar, bio, phone, latitude, longitude } = await req.json();
  ...
  const user = await prisma.user.update({
    where: { id: session.id },
    data,
    select: { id: true, name: true, email: true, avatar: true, bio: true, phone: true, latitude: true, longitude: true },
  });
