...
interface ClothingItem {
  id: string; title: string; description: string; size: string; brand: string;
  condition: string; category: string; imageUrl: string; price: number | null;
}
...
export default function EditItemPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (!user) return;
    fetch(`/api/clothes/${id}`).then(async (res) => {
      if (!res.ok) { setNotFound(true); setFetching(false); return; }
      const item: ClothingItem = await res.json();
      const brandKnown = (BRANDS as readonly string[]).includes(item.brand);
      const sizeKnown = sizesForCategory(item.category).includes(item.size);
      setForm({
        title: item.title,
        description: item.description,
        size: sizeKnown ? item.size : "Otro",
        brand: brandKnown ? item.brand : "Otra",
        condition: item.condition,
        category: item.category,
        price: item.price != null ? String(item.price) : "",
      });
      if (!brandKnown) setCustomBrand(item.brand);
      if (!sizeKnown) setCustomSize(item.size);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/clothes/${id}`, {
        ...
        body: JSON.stringify({ ...form, brand, size, imageUrl }),
      });
      const data = await res.json().catch(() => null);
      if (!res.ok) {
        throw new Error(data?.error ?? "No se pudo guardar los cambios. Intentá de nuevo.");
      }
  ...
  if (notFound) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-3 text-center px-4">
      <p className="text-sm text-slate-500">No encontramos esa prenda.</p>
      <button onClick={() => router.push("/ropero")} className="text-sm text-rose-500 font-semibold hover:underline">Volver al ropero</button>
    </div>
  );

  return (
    <div className="max-w-sm mx-auto px-4 pt-6 pb-6">
      <button onClick={() => router.push("/ropero")} className="flex items-center gap-1 text-slate-400 hover:text-slate-600 text-sm mb-4">
        <ArrowLeft size={16} /> Volver
      </button>
      <h1 className="text-xl font-bold text-slate-800 mb-5">Editar prenda</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <motion.button whileTap={{ scale: 0.97 }} type="submit" disabled={loading}
          ...
          <Upload size={18} /> {loading ? "Guardando..." : "Guardar cambios"}
        </motion.button>
      </form>
    </div>
  );
